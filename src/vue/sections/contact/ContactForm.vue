...
const _sendMessage = (values) => {
  ...
  //     }
  // }, 1000)
  //   useData().sectionData['content']['items']

  const email = data.getProfile().contact.email.value;
  const subject = "subject=" + values["subject"];
  let body = `body=${values["name"]} ${values["email"]}\n\n${values["message"]}`;

  body += "\n\n\nMail server not configured yet. Sent from portfolio website.";
